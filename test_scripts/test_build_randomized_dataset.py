...
class MyTestCase(unittest.TestCase):
    ...
    def test_get_root_dir(self):
        self.assertEqual(
            build_dataset._get_root_dir_path(
                self.test_path + 'mutacc_config_files/mutacc_config.yaml'),
            '/home/mire/PycharmProjects/project_test/mutacc_tests/')

    def test_extract_case_ids(self):
        caselist = build_dataset._extract_case_ids(self.test_path + 'mutacc_config_files/mutacc_config.yaml')
        print(caselist)
    ...
    def test_create_synthesized_dataset_from_database(self):
        background_bam = self.test_path + 'hg38/NA12878_mybam.sorted.bam'
        background_fastq1 = self.test_path + 'mutacc_NA12878_mybam_R1.fastq.gz'
        background_fastq2 = self.test_path + 'mutacc_NA12878_mybam_R2.fastq.gz'
        caselist = build_dataset._extract_case_ids(self.test_path + 'mutacc_config_files/mutacc_config.yaml')
        case_db_configfile = self.test_path + 'mutacc_config_files/mutacc_config.yaml'
        synth_db_configfile = self.test_path + 'mutacc_config_files/mutacc_synthDB_config.yaml'
    ...
    def test_create_randomized_dataset(self):
        ...
        case_db_configfile = self.test_path + 'mutacc_config_files/mutacc_config.yaml'
        synth_db_configfile = self.test_path + 'mutacc_config_files/mutacc_synthDB_config.yaml'
        ref_fastq_1 = self.test_path + 'mutacc_NA12767_mybam_R1.fastq.gz'
        ref_fastq_2 = self.test_path + 'mutacc_NA12767_mybam_R2.fastq.gz'

        build_dataset.create_randomized_dataset(case_db_configfile, synth_db_configfile, background_bam,
                                                background_fastq1, background_fastq2, ref_fastq_1, ref_fastq_2)
